...
struct Meta {
    ...
    wind_speed: ((f32, Option<f32>, Option<f32>), usize),
    automode_wind_speed: ((f32, Option<f32>, Option<f32>), usize),
    vertical_wind_direction: ((f32, Option<f32>, Option<f32>), usize),
    horizontal_wind_direction: ((f32, Option<f32>, Option<f32>), usize),
}
...
impl DaikinStatus {
    pub fn new(response: DaikinResponse) -> Self {
        DaikinStatus {
            ...
            wind_speed:
                get_prop!(response."/dsiot/edge/adr_0100.dgc_status".e_1002.e_3001.p_09 -> u8)
                    .map(|v| v.into()),
            automode_wind_speed:
                get_prop!(response."/dsiot/edge/adr_0100.dgc_status".e_1002.e_3001.p_26 -> u8)
                    .map(|v| v.into()),
            vertical_wind_direction:
                get_prop!(response."/dsiot/edge/adr_0100.dgc_status".e_1002.e_3001.p_05 -> u8)
                    .map(|v| v.into()),
            horizontal_wind_direction:
                get_prop!(response."/dsiot/edge/adr_0100.dgc_status".e_1002.e_3001.p_06 -> u8)
                    .map(|v| v.into()),
            meta: Meta {
...
impl std::convert::From<u8> for WindSpeed {
    fn from(num: u8) -> Self {
        match num {
            0x0B => Self::Silent,
            0x03 => Self::Lev1,
...
impl std::convert::From<u8> for AutoModeWindSpeed {
    fn from(num: u8) -> Self {
        match num {
            0x0B => Self::Silent,
            0x0A => Self::Auto,
...
impl std::convert::From<u8> for VerticalDirection {
    fn from(num: u8) -> Self {
        match num {
            0x01 => Self::TopMost,
            0x02 => Self::Top,
...
impl std::convert::From<u8> for HorizontalDirection {
    fn from(num: u8) -> Self {
        match num {
            0x02 => Self::LeftMost,
            0x03 => Self::Left,
            0x04 => Self::LeftCenter,
            0x05 => Self::Center,
            0x06 => Self::RightCenter,
            0x07 => Self::Right,
            0x08 => Self::RightMost,
            0x0F => Self::Swing,
            0x10 => Self::Auto,
...
mod tests {
    ...
    fn getter() {
        ...
        assert_eq!(status.vertical_wind_direction, Some(VerticalDirection::Auto));
        assert_eq!(status.horizontal_wind_direction, Some(HorizontalDirection::Auto));
    }
